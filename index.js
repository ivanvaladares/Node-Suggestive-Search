...
const NodeSuggestiveSearch = class {
	...
	constructor (options) {
		...
		if (options.dataBase.toLowerCase() == "mongodb" || 
			...
			options.dataBase.toLowerCase() == "mssql" || 
			options.dataBase.toLowerCase() == "mysql") {
			...
			this._db.cleanDatabase().then(() => {		
				...
				this._db.insertItem(itemsArray).then(itemsInserted => {
					...
					itemsInserted.map(itemObject => {
						...
						//in case item have and integer id, lets use it
						//let itemId = itemObject["id"] != undefined ? itemObject.id : itemObject.itemId;
						...
						arrWords.map(word => {
							...
							if (strWord in objWords) {
								...
								objWords[strWord].items[itemId] = 1;
